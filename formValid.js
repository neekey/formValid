 ...
 */
KISSY.add( 'formValid', function( K ){
...
var formValid = function( formId, callback, rules ){
	// ====以下为非函数成员变量====
	 ...
	 this.formEl = this.form.getDOMNode();
	 this.callback = callback;
	 if( rules ){
	 	this.rules = K.merge( this.rules, rules );
	 }
	 this.init();
};
formValid.prototype = {
	...
	init: function(){
		...
		for( i = 0; formChilds[ i ]; i++ ){
			el = formChilds[ i ];
			// 添加提交事件
			if( K.one( el ).attr( 'type' ) == 'submit' ){
				K.one( el ).on( 'click', function( ev ){
					if( !that.check() ){
						ev.halt();
						return false;
					}
			...
			// 添加change 事件
			K.one( el ).on( 'change', function(){
				that.check_field( this.getDOMNode() );
			});
			K.one( el ).on( 'blur', function(){
				that.check_field( this.getDOMNode() );
			});
			K.one( el ).on( 'keyup', function(){
				that.check_field( this.getDOMNode() );
			});
	...
	rules: {
		 ...
		 */
	   	match: {
		 	msg: "两次输入不匹配!",
		 	test: function(obj){
		   	var target = K.one(obj).attr('match');
		   		return !obj.value || (obj.value === K.one('#' + target).val());
		 	}
	   	},
	   	phone: {
...
K.namespace( 'formValid' );
K.formValid = formValid;
});
